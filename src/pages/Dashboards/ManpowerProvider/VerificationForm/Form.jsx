  ...
  return (
    <div className="fixed flex items-center justify-center z-50 inset-0">
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              className="w-full border rounded p-2 mt-2 mb-2 outline-none"
            />
          </div>

          <div>
            <label htmlFor="agencyAddress" className="sr-only">
              Agency Address
            </label>
            <input
              ...
              className="outline-none border rounded p-2 mt-2 mb-2 w-full"
            />
          </div>

          <div>
            <select
              onChange={(e) => setAgencyServicesOffered(e.target.value)}
              required
              value={agencyServicesOffered}
              name="agencyServicesOffered"
              id="agencyServicesOffered"
              className="w-full p-2 mb-2 mt-2 outline-none border rounded"
            >
          ...
          <div>
            <label htmlFor="agencyAuthorizedRepresentative" className="sr-only">
              Authorized Personnel
            </label>
            <input
              onChange={(e) => setAgencyAuthorizedRepresentative(e.target.value)}
              required
              value={agencyAuthorizedRepresentative}
              type="text"
              placeholder="Authorized Personel"
              id="agencyAuthorizedRepresentative"
              name="agencyAuthorizedRepresentative"
              className="outline-none p-2 mb-2 mt-2 border rounded w-full"
            />
          </div>
          ...
          <div className="mt-6">
            ...
            <FileUpload
              ...
              setPreview={(v) =>
                v === 'modal' ? setShowMayor_BirModal(true) : setMayor_birPrev(v)
              }
            ...
            <FileUpload
              ...
              setPreview={(v) =>
                v === 'modal' ? setShowAgency_ProofModal(true) : setAgency_proofPreview(v)
              }
              id="agency_proof"
              allowPreview
            />
            ...
            <FileUpload
              ...
              description="Upload a clear image of your ID"
              file={authorized_agency_id}
              setFile={setAuthorized_agency_id}
              preview={authorized_agencyPreview}
              setPreview={(v) =>
                v === 'modal'
                  ? setShowAuthorized_agencyPreview(true)
                  : setAuthorized_agencyPreview(v)
              }
      ...
      <PreviewImage
        show={showDoleModal}
        src={DOLEPreview}
        alt="DOLE Certificate Preview"
        onClose={() => setShowDoleModal(false)}
      />
      <PreviewImage
        show={showMayor_BirModal}
        src={mayor_birPrev}
        alt="Mayor/BIR Preview"
        onClose={() => setShowMayor_BirModal(false)}
      />
      <PreviewImage
        show={showAgency_ProofModal}
        src={agency_proofPreview}
        alt="Agency Proof Preview"
        onClose={() => setShowAgency_ProofModal(false)}
      />
      <PreviewImage
        show={showAuthorized_agencyPreview}
        src={authorized_agencyPreview}
        alt="Authorized Agency ID Preview"
        onClose={() => setShowAuthorized_agencyPreview(false)}
      />
    </div>
  );
};
