...
import { useManpowerPosts } from '../../../../../hooks/useManpowerPosts';
...
const FindWorkers = () => {
  ...
  const handleShowSuccess = (workerDetails) => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <div className="flex-1 mx-auto space-y-4 w-full">
        {isLoading ? (
          <div className="p-5 text-center">Loading manpower job posts...</div>
        ) : isError ? (
          <div className="p-5 text-center text-red-600">Failed to load manpower posts.</div>
        ) : currentData.length === 0 ? (
          <div className="p-5 text-center text-gray-500">No job posts available.</div>
        ) : (
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <h2 className="text-3xl font-bold text-gray-900 mb-1">{worker.worker_category}</h2>
                  <p className="text-[#828282] mb-2">
                    ...
                    {worker.workersAvailable && (
                      <>
                        {' '}
                        | <span>{worker.workersAvailable}</span>
                      </>
                    )}
                  </p>
