...
const JobpostDetails = ({ jobPost, onClose }) => {
  ...
  const getSubmitterName = () => {
    if (jobPost.role === ROLE.BUSINESS_EMPLOYER)
      return jobPost.authorized_person || 'Unknown Business';
    if (jobPost.role === ROLE.INDIVIDUAL_EMPLOYER) return jobPost.full_name || 'Unknown Individual';
    if (jobPost.role === ROLE.MANPOWER_PROVIDER)
      return jobPost.agency_authorized_person || 'Unknown Agency';
    return 'Unknown';
  };
    ...
    <div className="fixed inset-0 z-50 bg-opacity-50 flex items-center justify-center">
      ...
      >
        ...
        <div className="justify-between my-4 space-y-5">
          <p>
            <span className="font-semibold">Title:</span> {jobPost.job_title}
          </p>
          <p className="wrap-break-word whitespace-pre-wrap">
            <span className="font-semibold">Location:</span> {jobPost.location}
          </p>
          <p>
            <span className="font-semibold">Job Type:</span> {jobPost.job_type}
          </p>
          <p className="wrap-break-word whitespace-pre-wrap">
            <span className="font-semibold">Required Skill:</span> {jobPost.required_skill}
          </p>
          <p>
            <span className="font-semibold">Salary:</span> {jobPost.salary_range}
          </p>
          <p>
            <span className="font-semibold">Verified:</span> {jobPost.created_at}
          </p>
          <p>
