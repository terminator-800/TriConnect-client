    ...
    <div className="overflow-y-auto w-[500px]">
      <ul className="divide-y divide-gray-200">
        {users.map((user) => {
          ...
          const fullName = user.name || 'unknown' ||  'loading...';
          const initials = getInitials(fullName);
          
          return (
            <li
              key={user.conversation_id}
              className={`w-full p-4 cursor-pointer flex justify-between items-start gap-3
                           border-gray-300 border-l-4 md:border-b border-y
                          ${isSelected
            ...
            >
              ...
              <div className="flex gap-2 flex-1 max-w-10">
                {/* Profile || LOGO */}
                <div className="w-10 h-10 rounded-full shrink-0 overflow-hidden flex items-center justify-center bg-[#8B5CF6] text-white text-sm">
                  {user.profile ? (
                    <img
                      src={user.profile}
                ...
                <div>
                  <div className="font-bold text-lg truncate max-w-50 ">{user.name || 'Unknown Name'}</div>
