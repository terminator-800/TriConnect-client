  ...
  ({ label, description, setFile, preview, setPreview, id, allowPreview = false }, ref) => {
    ...
    const handleFileChange = (e) => {
      ...
      if (!validImageTypes.includes(selectedFile.type)) {
        alert('Invalid file type. Please upload a valid image file (JPEG, PNG or JPG).');
        if (fileInputRef.current) fileInputRef.current.value = '';
        return;
      }
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="text-sm cursor-pointer bg-blue-900 px-5 py-2 rounded-xl text-white hover:bg-blue-800 transition-colors"
              >
                Reupload
              </button>
              <button
                type="button"
                onClick={startCamera}
                className="text-sm cursor-pointer bg-green-700 px-5 py-2 rounded-xl text-white hover:bg-green-600 transition-colors"
              >
                Retake
              </button>
              <button
                ...
                className="text-sm cursor-pointer bg-red-500 px-5 py-2 rounded-xl text-white hover:bg-red-600 transition-colors"
              >
                Cancel
              </button>
            </div>
          </div>
          <input ref={fileInputRef} onChange={handleFileChange} type="file" name={id} id={id} accept="image/png, image/jpeg, image/jpg" className="hidden" />
        </div>
      );
    }
      ...
      <div className="border-2 border-gray-400 rounded px-10 py-5 flex flex-col items-center justify-center text-center mb-5 gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full justify-center">
          <label htmlFor={id} className="cursor-pointer">
            <div className="border border-gray-400 rounded-xl flex items-center justify-center gap-3 px-5 py-2 hover:bg-gray-50 transition-colors">
          ...
          <button
            type="button"
            onClick={startCamera}
            className="border border-gray-400 rounded-xl flex items-center justify-center gap-3 px-5 py-2 hover:bg-gray-50 transition-colors cursor-pointer"
          >
            <span className="text-lg">📷</span>
            <span className="text-gray-500 text-sm">Use Webcam</span>
          </button>
        </div>
