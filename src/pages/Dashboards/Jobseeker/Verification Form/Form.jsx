...
const Form = ({ onClose, onSubmitSuccess }) => {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return axios.post(
        `${import.meta.env.VITE_API_URL}/${ROLE.JOBSEEKER}/upload-requirements`,
        formData,
        {
    ...
    <div className="fixed flex items-center justify-center z-50 inset-0 h-[90vh]">
      {!showSuccessModal && (
        ...
        >
          ...
          <div className="mb-3">
            <label htmlFor="jsFullname" className="sr-only">
              Full Name
            </label>
            <input
              onChange={(e) => setJsFullName(e.target.value)}
              required
              value={jsFullname}
              type="text"
          ...
          <div className="mb-3">
            <label htmlFor="jsDob" className="block text-sm font-medium text-gray-700 sr-only">
              Date of Birth
            </label>
            <input
              onChange={(e) => setJsDob(e.target.value)}
              required
              value={jsDob}
              type="date"
              id="jsDob"
              name="jsDob"
              className="border p-2 rounded outline-none w-1/2"
              placeholder="Date of Birth"
            />
          </div>

          <div className="mb-3">
            <label htmlFor="jsContactNumber" className="sr-only">
              Jobseeker Contact Number
            </label>
            <input
              onChange={(e) => setJsContactNumber(e.target.value)}
              required
              value={jsContactNumber}
              type="number"
              placeholder="Contact Number"
              id="jsContactNumber"
              name="jsContactNumber"
              className="border rounded p-2 mt-2 mb-2 w-1/2 outline-none"
            />
          </div>

          <div className="mb-3">
            <select
              onChange={(e) => setJsGender(e.target.value)}
              required
              value={jsGender}
              id="jsGender"
              name="jsGender"
          ...
          <div className="mb-3">
            <label htmlFor="jsPresentAddress" className="sr-only">
              Jobseeker Present Address
            </label>
            <input
              onChange={(e) => setJsPresentAddress(e.target.value)}
              required
              value={jsPresentAddress}
              type="text"
              placeholder="Present Address"
              id="jsPresentAddress"
              name="jsPresentAddress"
              className="border rounded w-full p-2 mt-2 mb-2 outline-none"
            />
          </div>

          <div className="mb-3">
            <label htmlFor="jsPermanentAddress" className="sr-only">
              Jobseeker Permanent Address
            </label>
            <input
              onChange={(e) => setJsPermanentAddress(e.target.value)}
              value={jsPermanentAddress}
              type="text"
              placeholder="Permanent Address (optional if same as present)"
              id="jsPermanentAddress"
              name="jsPermanentAddress"
              className="border rounded w-full p-2 mt-2 mb-2 outline-none"
            />
          </div>
          ...
          <div className="mb-3">
            <select
              onChange={(e) => setJsEducationalAttainment(e.target.value)}
              required
              value={jsEducationalAttainment}
              name="jsEducationalAttainment"
              id="jsEducationalAttainment"
          ...
          <div className="mt-15">
            ...
            <FileUpload
              ...
              preview={
                jsGovernmentIDPreview === 'modal' ? jsGovernmentIDPreview : jsGovernmentIDPreview
              }
              setPreview={(value) =>
                value === 'modal' ? setShowGovIDModal(true) : setJsGovernmentIDPreview(value)
              }
              id="jsGovernmentID"
              allowPreview
            />
            ...
            <FileUpload
              ...
              setPreview={(value) =>
                value === 'modal' ? setShowSelfieModal(true) : setSelfieIDPreview(value)
              }
              id="selfieID"
              allowPreview
            />
            ...
            <FileUpload
              ...
              setPreview={(value) =>
                value === 'modal' ? setShowClearanceModal(true) : setClearancePreview(value)
              }
              id="clearanceFile"
              allowPreview
            />
