...
export const useCreateHiringJobPost = (role, onSuccessCallback) => {
  ...
  return useMutation({
    ...
    onError: (error) => {
      ...
      } else if ((status === 400 || status === 400) && message?.includes('only create up to')) {
        alert(
          'You’ve reached the limit of 3 active job posts. Please remove an existing one or upgrade your plan to continue.'
        );
      } else {
  ...
  return useMutation({
    ...
    onError: (error) => {
      ...
      if (status === 400 && message?.includes('maximum')) {
        ...
        alert(
          'You’ve reached the limit of active team job posts. Please remove an existing one or upgrade your plan to continue.'
        );
      } else {
